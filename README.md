...
- function types: () -> (), (ParamType, ParamType) -> Return Type
- can be nested

# Closures
- reversed = sort(array, { (s1: String, s2: String) -> BOOL in return s1 > s2 })
- with type inferene: reversed = sort(array, { s1, s2 in return s1 > s2 })
...
- associated values: enum EnumName {
	case Foo(Int, Int)
	case Bar(String)
}
...
- raw values: enum ASCIIControlCharacter: Character {
	case Tab = "\t"
	case LineFeed = "\n"
...
- adding protocol with extension: extension SomeClass: SomeProtocol {
	func requiredMethod() {
		// do stuff
	}
}
...
- associated types: used in protocols
	...
	- conforming: typealias ItemType = Int, could also be a generic like T, swift can frequently infer this though
...
- custom operators
	...
	- custom infix operators can specify precedence and associativity, operator infix +- { associativity left precedence 140 }